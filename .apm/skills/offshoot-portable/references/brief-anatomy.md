...
and one call from this session costs the user another half hour.*
...
> `deciders: <owner>`, and stop there. Do not implement past the record and do not mark it
> accepted. The user decides, and a proposed record is the correct artefact for a decision
...
> the user first, a short honest document saying so is a better outcome than speculative
...
with no calls from that API key, and a single call from this session costs the user another
half hour.
...
record at `status: proposed` with `deciders: <owner>`, name the alternatives you considered
